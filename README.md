...
특이한 점은 마우스를 뗀 후 실행할 함수는 `onMouseUp` 가 아닌 `onClick` 에서 트리거됩니다. 클릭을 뗄 때 `onMouseUp` 과 `onClick` 은 모두 트리거되지만 <u>`onClick` 은 마우스 커서가 컴포넌트에서 벗어나지 않아야 한다</u>는 차이점이 있습니다.

그래서 드래그 중에 마우스가 창에서 벗어날 때 변경사항이 저장되지 않도록 제약을 걸 수 있었습니다.
